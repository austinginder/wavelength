...
| `export <job.json> [--out song.mid \| song.dawproject] [--json]` | Writes the job's parts as a type 1 MIDI file to open in a DAW or notation program: tempo map (ramps stepped every 1/16 beat), time signature, markers, and a track per job track with its notes, fader, pan, rides and CC, pitch bend and pressure automation. Export, import and export again gives the same file byte for byte. To a `.dawproject` it writes a project Bitwig Studio (and Studio One, Cubase) opens with the song's plugins loaded: each plugin's state as the job sets it up (preset, state file, params; saved in worker processes), notes, faders, pans, mutes, sends, buses as effect tracks, the master, tempo map, markers, fader and pan curves, plain audio clips, and built-in eq/compressor/limiter as DAWproject's standard devices. Tracks with built-in instruments are printed: rendered dry to audio on the track, next to their notes (`--no-print` keeps only the notes). Other built-in effects are listed as left out. |
