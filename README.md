...
| `import <song.dawproject> [--out DIR] [--json]` | Turns a DAWproject export (Bitwig, Studio One, Cubase) into a job: notes, tracks with their plugins and saved states, mixer, sends, groups, tempo map, markers, volume and pan automation. Lists what the file can't carry (a DAW's own devices and samples, launcher clips, audio tracks). `render song.dawproject` imports and renders in one step. |
