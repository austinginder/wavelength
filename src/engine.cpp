...
clap_event_param_value_t paramEvent(clap_id id, void *cookie, double value, uint32_t at) {
...
bool openPlugin(const PluginSetup &setup, const std::string &context, OpenedPlugin &out, std::string &err) {
    ...
    if (!bundle) { err = context + ": " + err; return false; }
    out.inst = Instance::create(bundle, info.id, err);
    if (!out.inst) { err = context + ": " + err; return false; }
    out.inst->verbose = setup.verbose;

    if (!setup.stateFile.empty()) {
        StateFile sf;
        if (!readStateFile(setup.stateFile, setup.stateFormat, sf, err)) { err = context + ": " + err; return false; }
        if (!sf.pluginId.empty() && sf.pluginId != info.id) {
            err = context + ": state file " + setup.stateFile + " belongs to '" + sf.pluginId + "', not '" + info.id + "'";
            return false;
        }
        out.stateFormat = sf.format;
        if (!out.inst->loadState(sf.state, err)) { err = context + ": " + err; return false; }
        out.inst->pump(50);
    }
    auto lookup = [&](const std::string &key, ParamInfo &pi) -> bool {
        if (out.inst->findParam(key, pi)) return true;
        err = context + ": no parameter '" + key + "' on " + info.name + " (run `wavelength params " + info.id + "`)";
        return false;
    };
    for (const auto &p : setup.params) {
        ParamInfo pi;
        if (!lookup(p.key, pi)) return false;
        const double v = std::clamp(p.value, pi.min, pi.max);
    ...
    for (const auto &[key, env] : setup.automation) {
        ...
        out.initial.push_back({pi.id, pi.cookie, std::clamp(env.at(0), pi.min, pi.max)});
    }
    if (!out.inst->setParams(out.initial, err)) { err = context + ": " + err; return false; }
    return true;
}
...
bool runPlugin(const Job &job, OpenedPlugin &p, const std::vector<TimedEvent> &events, const Audio *input, Audio &out,
    ...
    if (input && inPorts == 0) { inst.deactivate(); err = p.name + " has no audio input, so it can't be used as an effect"; return false; }
    ...
    std::thread worker([&] {
        ...
        std::vector<double> lastAuto(p.autos.size(), NAN);
        ...
        for (int64_t pos = -warm; pos < total && audioErr.empty();) {
            ...
            params.reserve(p.initial.size() + p.autos.size());
            Events in;
            if (first) {   // parameter values again through process(): some plugins only commit them here
                for (const auto &v : p.initial) params.push_back(paramEvent(v.id, v.cookie, v.value, 0));
                first = false;
            }
            if (pos >= 0) {
                const double t = pos / sr;
                for (size_t a = 0; a < p.autos.size(); ++a) {
                    const double v = p.autos[a].env.at(t);
                    if (!(std::fabs(v - lastAuto[a]) < 1e-9)) { params.push_back(paramEvent(p.autos[a].id, p.autos[a].cookie, v, 0)); lastAuto[a] = v; }
                }
            }
    ...
    for (auto &line : inst.log)
        if (line.rfind("error", 0) == 0 || line.rfind("fatal", 0) == 0 || line.rfind("warning", 0) == 0) p.warnings.push_back("plugin " + line);
