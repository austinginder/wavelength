...
| `repeat` | beat repeat / stutter: `on` 0 *(automatable: at each rise past 0.5 the incoming audio is captured and its first `size` beats loop while `on` stays up)*, `size` 0.25 beats *(automatable: a change while on restarts the loop at the new size, so steps 1/4 -> 1/8 -> 1/16 -> 1/32 roll)*, `fade` 2 ms (edges of each repeat), `mix` 1 *(automatable)*. On a bus or the master it repeats reverb and delay tails with the rest: `{"type": "repeat", "size": 0.25, "on": 0, "automate": {"on": [[0, 0], [60, 0, "step"], [60, 1], [64, 1, "step"], [64, 0]], "size": [[0, 0.5], [62, 0.5, "step"], [62, 0.25], [63, 0.25, "step"], [63, 0.125]]}}` |
