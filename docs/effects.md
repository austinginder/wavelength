...
| `clip` | `ceiling` -1 dB, `knee` 0.5 (fraction of the ceiling where the curve starts: 0.01 hard, 1 very soft), `drive` 0 dB *(automatable)*. A soft clipper that never exceeds the ceiling: put it before the master `limiter` (or on a kick or clap) to shave the first milliseconds of transients instead of limiting the whole mix. Warns when it shapes so much that it's distortion. |
